      ...
      <div className="phone-mockup">
        ...
        <div className="phone-screen">
          ...
          <div className="phone-product">
            <div className="phone-product-img">{preview?.image_url?<img src={preview.image_url} alt=""/>:<span>🛍️</span>}</div>
            <div><small>{preview?.category_name||'Anúncio local'}</small><b>{preview?.title||'Seu produto aqui'}</b><strong>{preview?.price?Number(preview.price).toLocaleString('pt-BR',{style:'currency',currency:'BRL'}):'R$ 0,00'}</strong></div>
          </div>
        </div>
      </div>
