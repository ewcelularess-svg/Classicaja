...
import {PauseCircle, PlayCircle, PlusCircle, Sparkles, Trash2} from 'lucide-react';
...
export default function MyAds(){
 ...
 {err?<div className="empty"><h3>{err}</h3></div>:items.length?<div className="manage-list">{items.map(p=><div className="manage-card advanced" key={p.id}>
  ...
  <div className="manage-info"><Link to={`/produto/${p.id}`}><b>{p.title}</b></Link><span>{money(p.price)}</span><small>{p.neighborhood?`${p.neighborhood}, `:''}{p.city} - {p.state} • {p.views} visualizações</small><div className="status-line"><span className={`status-dot ${p.status}`}></span>{p.status}{p.featured_active&&<em>• Em destaque</em>}</div></div>
  <div className="manage-actions"><Link className="boost-btn" to={`/destaque/${p.id}`}><Sparkles/> Destacar</Link><button className="secondary-btn" onClick={()=>toggle(p)}>{p.status==='active'?<PauseCircle/>:<PlayCircle/>}{p.status==='active'?'Pausar':'Ativar'}</button><button className="danger" onClick={()=>del(p.id)}><Trash2/> Excluir</button></div>
